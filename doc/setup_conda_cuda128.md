...
* A linux 64-bits architecture (i.e. `linux-64` platform in `conda info`).
* A NVIDIA GPU with at least 32 Gb of VRAM.

## 1. Setup Python Environment

The following will install the default environment using conda. Note that you may have to build the environment on a compute node with GPU (e.g., you may get a `RuntimeError: Not compiled with GPU support` error when running certain parts of the code that use Pytorch3D).
...
# for pytorch/cuda dependencies (CUDA 12.8)
...
# for inference (PyTorch 2.7 with CUDA 12.8)
export PIP_FIND_LINKS="https://nvidia-kaolin.s3.us-east-2.amazonaws.com/torch-2.7_cu128.html"
